                ...
                <table class="table" style="border: 1px solid white;">
                    ...
                    <tbody>
                        @foreach ($transaksiModel as $item)
                            ...
                            {{-- Modal Ubah Status --}}
                            <div class="modal fade" id="ubahStatusModal{{ $item->id }}" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form action="{{ route('transaksi.update', ['id' => $item->id]) }}"
                                                method="post">
                                                @csrf
                                                @method('PUT')
                            ...
                            <div class="modal fade" id="detailModal{{ $item->id }}" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="detailModalLabel">Detail Transaksi <b>ID
                                                    {{ $item->id }}</b></h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                                ...
                                                @if ($item->detail_transaksi)
                                                    @foreach ($item->detail_transaksi as $detailTransaksi)
                                                        <li>{{ $detailTransaksi->nama_produk }} - <b>Qty :
                                                                <span>{{ $detailTransaksi->qty }}</span> </b></li>
                                                        <li class="form-control">Rp.
                                                            {{ number_format($detailTransaksi->harga, 0, ',', '.') }}</li>
                                                    @endforeach
                                                @else
